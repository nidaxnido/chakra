...
function AddTodo({toSubmit}: {toSubmit  : (item:Todo) => void}){
    ...
    function doSubmit(e : React.FormEvent){
        ...
        localStorage.setItem('add',content);
        const newItem:Todo = {
            id : Math.random(),
            body : content
        }
...
function Todo(){
    ...
    function setAwal(key:string){
        const [value, useValue] = useState(initial);
        useEffect(() => {
            const stored = localStorage.getItem(key);
            useValue(stored ? JSON.parse(stored) : []);
        }, []);
        // useEffect(() => {
    ...
    const [todos, setTodos] = setAwal('todos');
